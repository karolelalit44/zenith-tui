...
function extractEventKindFromTS(src: string): string[] {
  ...
  const re = /\|\s+'([^']+)'/g;
  let match = re.exec(kindBlock[0]);
  while (match !== null) {
    kinds.push(match[1]);
    match = re.exec(kindBlock[0]);
  }
  return kinds;
}
...
function checkBackendEventKinds() {
  ...
  pass(
    'BE-1: Backend EventKind enum',
    `Found ${backendKinds.length} kinds in Python backend: ${backendKinds.join(', ')}`,
  );
  return backendKinds;
}
...
function checkFrontendEventKinds() {
  ...
  pass(
    'FE-1: Frontend EventKind type',
    `Found ${frontendKinds.length} kinds in frontend types: ${frontendKinds.join(', ')}`,
  );
...
function checkAlignment(backendKinds: string[], frontendKinds: string[]) {
  ...
  if (missingInFrontend.length === 0 && missingInBackend.length === 0) {
    pass(
      'ALIGN-1: Backend ↔ Frontend 1:1 mapping',
    ...
    if (missingInFrontend.length) {
      fail('ALIGN-1: Backend ↔ Frontend', `Backend kinds missing in frontend: ${missingInFrontend.join(', ')}`);
    }
    if (missingInBackend.length) {
      fail('ALIGN-1: Backend ↔ Frontend', `Frontend kinds missing in backend: ${missingInBackend.join(', ')}`);
    }
  }
}
...
function checkEventMapper(backendKinds: string[]) {
  ...
  if (unhandled.length === 0 && extraCases.length === 0) {
    pass(
      'MAP-1: EventMapper handles all kinds',
      `All ${backendKinds.length} EventKind types handled in EventMapper switch statement`,
    );
  } else {
    if (unhandled.length) {
      fail('MAP-1: EventMapper missing cases', `Backend kinds not handled by EventMapper: ${unhandled.join(', ')}`);
    }
    if (extraCases.length) {
      warn(
        'MAP-1: EventMapper extra cases',
  ...
  if (mapperSrc.includes('default:')) {
    pass('MAP-2: EventMapper has default fallback', 'Unknown event kinds fall back to message event with JSON dump');
  } else {
    fail('MAP-2: EventMapper missing default case', 'No default case — unknown kinds would crash the mapper');
  }
}
...
function checkComponentRegistry(backendKinds: string[]) {
  ...
  if (registrySrc.includes('UnknownEventFallback')) {
    pass('REG-2: Component Registry has fallback', 'Unknown event kinds render via UnknownEventFallback component');
  } else {
    fail('REG-2: Component Registry missing fallback', 'No fallback component for unregistered event kinds');
  }
}
...
function checkPipelineIntegrity(backendKinds: string[], frontendKinds: string[]) {
  ...
  if (beDups.length === 0 && feDups.length === 0) {
    pass('INT-1: No duplicate EventKind values', 'Backend and frontend event kind lists contain no duplicates');
  } else {
  ...
  // Verify exact count
  if (backendKinds.length === 20 && frontendKinds.length === 20) {
    pass('INT-2: EventKind count (20)', `Backend: ${backendKinds.length}, Frontend: ${frontendKinds.length}`);
  } else {
    warn(
      'INT-2: EventKind count',
      `Expected 20, got Backend: ${backendKinds.length}, Frontend: ${frontendKinds.length}`,
    );
  }
  ...
  if (JSON.stringify(beSorted) === JSON.stringify(feSorted)) {
    pass('INT-3: EventKind values identical after sort', 'Both sides have exactly the same set of event kinds');
  } else {
    warn('INT-3: EventKind values differ', 'Backend and frontend have different event kind sets — see ALIGN-1');
  }
}
...
function checkStaleDist() {
  ...
  if (fs.existsSync(distDir)) {
    ...
    if (files.length > 0) {
      warn(
        'BUILD-1: Stale dist/ files',
        `Found ${files.length} compiled JS files in dist/. tsx may resolve .js over .tsx — run "npm run clean" to remove them. First 5: ${files
          .slice(0, 5)
  ...
  if (tsconfig?.compilerOptions?.outDir && pkg?.main) {
    ...
    if (resolvedOut.startsWith(srcDir) || path.relative(SRC, resolvedOut).startsWith('..') === false) {
      warn(
        'BUILD-2: tsconfig outDir conflicts with source tree',
        `outDir=${outDir} is inside src/ tree — tsx may resolve .js over .tsx`,
      );
    } else {
      pass(
        'BUILD-2: tsconfig outDir isolated from source',
...
function checkModuleResolution() {
  ...
  if (fs.existsSync(distComponentDir)) {
    const jsFilesInDist = fs
      .readdirSync(distComponentDir)
      .filter((f) => f.endsWith('.js'))
      .map((f) => f.replace('.js', ''));
    const conflicts = [...tsxFiles].filter((f) => jsFilesInDist.includes(f));
    if (conflicts.length > 0) {
      warn(
        'MOD-1: Competing .tsx ↔ .js files',
        `tsx may resolve .js over .tsx for: ${conflicts.join(', ')}. Run "npm run clean".`,
      );
    } else {
      pass('MOD-1: No competing module resolutions', 'No name conflicts between .tsx source and .js compiled files');
    }
  } else {
...
function checkUIQuality() {
  ...
  if (emojiCount === 0) {
    pass('UI-1: Zero emoji in UI components', 'All 22 scenario components use clean CLI text badges (no emoji)');
  } else {
    fail('UI-1: Emoji found in UI components', `Found ${emojiCount} emoji in files: ${filesWithEmoji.join(', ')}`);
  }
  ...
  if (singleBorders > 0 && roundBorders === 0) {
    pass('UI-2: Consistent border style', `All ${singleBorders} borders use "single" style (clean terminal look)`);
    ...
    warn(
      'UI-2: Mixed border styles',
      `Found ${singleBorders} "single" and ${roundBorders} "round" borders. Consider standardizing on "single".`,
    );
  } else {
...
function checkDataIntegrity() {
  ...
  pass(
    'DATA-1: Defensive string coercion',
  ...
  if (progressCase?.[0].includes('as ScenarioEvent')) {
    pass('DATA-4: Type assertion safety', 'EventMapper casts via "as ScenarioEvent" for type narrowing');
  }
}
...
function checkStreamHandling() {
  ...
  if (providerSrc.includes('partialMessageIndex')) {
    pass(
      'STREAM-1: Partial message streaming',
  ...
  if (providerSrc.includes('abort')) {
    pass('STREAM-3: Abort/cancel support', 'BackendScenarioProvider returns abort() function for cancellation');
  } else {
  ...
  if (providerSrc.includes('disconnected')) {
    pass('STREAM-4: Connection loss handling', 'BackendScenarioProvider emits error on WebSocket disconnect');
  } else {
  ...
  if (providerSrc.includes('Waiting for backend response')) {
    pass('STREAM-5: Waiting indicator', 'BackendScenarioProvider shows waiting indicator after 2s timeout');
  } else {
...
function checkComponentPropsAlignment() {
  ...
  if (propsIssues === 0) {
    pass(
      'PROPS-1: All event components have typed event props',
      `All ${propsOk} event-bound components define proper interfaces with event field`,
    );
  } else {
    warn(
      'PROPS-1: Some components lack event props',
