...
Includes an expanded list of banned commands (40+) matching Crush's safety
profile, covering network tools, system administration, package managers,
and system modification commands.
"""
...
# Banned commands — these are never allowed
...
def _check_command_chaining(command: str) -> bool:
    """Check if a command contains chaining operators (&&, ||, ;, |)."""
...
_LOW_RISK_PATTERNS: list[tuple[str, str]] = [
    ...
    (r'\bnpm\s+run\s+test\b', 'Test command'),
    (r'\bnpx\b', 'npx command execution'),
    (r'\bpython\s+.*\.py\b', 'Python script execution'),
    (r'\bpython3\s+.*\.py\b', 'Python script execution'),
    (r'\bnode\s+.*\.js\b', 'Node script execution'),
    (r'\bgo\s+run\b', 'Go run command'),
    (r'\bcargo\s+run\b', 'Cargo run command'),
    (r'\bcargo\s+build\b', 'Cargo build command'),
    (r'\bcargo\s+test\b', 'Cargo test command'),
]


def is_command_banned(command: str) -> str | None:
    """Check if a command uses a banned tool. Returns the banned command name or None."""
    # Extract the first command (handle pipes, chaining)
    first_cmd = command.strip().split()[0] if command.strip() else ""
    first_cmd = first_cmd.split("/")[-1]  # Handle full paths like /usr/bin/curl
...
def assess_command(command: str) -> RiskAssessment:
    ...
    any high or medium risk pattern, or uses a banned command.
    """
    normalized = command.strip().lower()

    # Check for banned commands first
    banned = is_command_banned(command)
    if banned:
        return RiskAssessment(
            is_risky=True,
            reason=f"Command '{banned}' is not allowed",
            risk_level="high",
        )
