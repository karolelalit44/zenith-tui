...
class GitOps:
    ...
    def get_prompt_context(self) -> str:
        """Build a git context block for injection into the system prompt.

        Returns a formatted string with branch, status summary, and recent
        commits, or empty string if not a git repo.  Keeps the output short
        (~10 lines max) so it doesn't consume excessive prompt tokens.
        """
        ...
        # Status summary (short, max 20 lines)
