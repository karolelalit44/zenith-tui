...
import { TestExecutionEvent } from '../../../types/scenario';
  ...
  return (
    <Box flexDirection="column" width="100%" marginBottom={1} paddingX={1}>
      <Box flexDirection="row" alignItems="center" marginBottom={1} flexWrap="wrap">
        <Text color="#3FB950" bold>[TEST]</Text>
        <Text color={theme.colors.text.muted}> </Text>
        <Text color="#58A6FF" bold>{event.framework}</Text>
        <Text color={theme.colors.text.muted}> · </Text>
      ...
      <Box flexDirection="column" paddingLeft={2}>
        {event.results.map((result, idx) => (
          <Box key={idx} flexDirection="row" alignItems="center">
            <Box width={2}>
              <Text color={result.status === 'pass' ? theme.colors.text.emerald : result.status === 'fail' ? theme.colors.text.error : theme.colors.text.muted}>
                {result.status === 'pass' ? '✔' : result.status === 'fail' ? '✗' : '○'}
              </Text>
            </Box>
            <Text color={result.status === 'fail' ? theme.colors.text.error : theme.colors.text.ethereal}>
              {result.name}
            </Text>
      ...
      <Box flexDirection="row" marginTop={1} paddingLeft={2}>
        ...
        <Text color={theme.colors.text.emerald}>{event.summary.passed} passed</Text>
        {event.summary.failed > 0 && (
          <>
            <Text color={theme.colors.text.muted}> · </Text>
            <Text color={theme.colors.text.error}>{event.summary.failed} failed</Text>
          </>
        )}
