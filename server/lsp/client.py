...
DEFAULT_SERVERS: dict[str, dict[str, Any]] = {".py": {"command": "pyright-langserver", "args": ["--stdio"], "name": "pyright"}, ".js": {"command": "typescript-language-server", "args": ["--stdio"], "name": "typescript-language-server"}, ".ts": {"command": "typescript-language-server", "args": ["--stdio"], "name": "typescript-language-server"}, ".tsx": {"command": "typescript-language-server", "args": ["--stdio"], "name": "typescript-language-server"}, ".jsx": {"command": "typescript-language-server", "args": ["--stdio"], "name": "typescript-language-server"}, ".go": {"command": "gopls", "args": [], "name": "gopls"}, ".rs": {"command": "rust-analyzer", "args": [], "name": "rust-analyzer"}, ".java": {"command": "jdtls", "args": [], "name": "jdtls"}, ".rb": {"command": "solargraph", "args": ["stdio"], "name": "solargraph"}, ".c": {"command": "clangd", "args": [], "name": "clangd"}, ".cpp": {"command": "clangd", "args": [], "name": "clangd"}, ".h": {"command": "clangd", "args": [], "name": "clangd"}}

_EXT_TO_LANG_ID: dict[str, str] = {".py": "python", ".js": "javascript", ".ts": "typescript", ".tsx": "typescriptreact", ".jsx": "javascriptreact", ".go": "go", ".rs": "rust", ".java": "java", ".rb": "ruby", ".c": "c", ".cpp": "cpp", ".h": "c", ".hpp": "cpp", ".cs": "csharp", ".php": "php", ".swift": "swift", ".kt": "kotlin", ".scala": "scala", ".html": "html", ".css": "css", ".scss": "scss", ".json": "json", ".yaml": "yaml", ".yml": "yaml", ".md": "markdown", ".sql": "sql", ".sh": "shellscript", ".bash": "shellscript", ".xml": "xml"}
...
class LspClient:
    ...
    def __init__(self, name: str, command: str, args: list[str] | None = None, cwd: str | None = None, env: dict[str, str] | None = None) -> None:
        self.name = name
    ...
    async def start(self) -> None:
        ...
        self._process = await asyncio.create_subprocess_exec(self.command, *self.args, stdin=asyncio.subprocess.PIPE, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, cwd=self.cwd, env=full_env)
    ...
    async def initialize(self, root_uri: str, root_path: str) -> dict:
        params = {"processId": os.getpid(), "rootUri": root_uri, "rootPath": root_path, "capabilities": {"textDocument": {"synchronization": {"dynamicRegistration": False, "willSave": False, "didSave": True, "willSaveWaitUntil": False}, "publishDiagnostics": {}, "definition": {"dynamicRegistration": False}, "references": {"dynamicRegistration": False}, "hover": {"dynamicRegistration": False}, "rename": {"dynamicRegistration": False}, "completion": {"dynamicRegistration": False, "completionItem": {"snippetSupport": False}}}}, "initializationOptions": {}}
    ...
    async def did_change(self, file_path: str, content: str, version: int | None = None) -> None:
        ...
        params = {"textDocument": {"uri": uri, "version": ver}, "contentChanges": [{"text": content}]}
            ...
            if notif.get("method") == "textDocument/publishDiagnostics":
                ...
                if diag_uri == Path(file_path).as_uri():
                    for d in notif.get("params", {}).get("diagnostics", []):
                        diagnostics.append({"range": d.get("range", {}), "severity": _severity_name(d.get("severity", 1)), "message": d.get("message", ""), "source": d.get("source", ""), "code": d.get("code", "")})

        await self.did_close(file_path)
        return diagnostics

    async def goto_definition(self, file_path: str, content: str, line: int, character: int) -> list[dict]:
        language_id = get_language_id(file_path)
        await self.did_open(file_path, language_id, content)
        ...
        for loc in locations:
            if "targetUri" in loc:
                definitions.append({"file": _uri_to_path(loc["targetUri"]), "line": loc.get("targetRange", loc.get("range", {})) .get("start", {}) .get("line", 0), "character": loc.get("targetRange", loc.get("range", {})) .get("start", {}) .get("character", 0)})
            elif "uri" in loc:
                definitions.append({"file": _uri_to_path(loc["uri"]), "line": loc.get("range", {}).get("start", {}).get("line", 0), "character": loc.get("range", {}).get("start", {}).get("character", 0)})
        return definitions

    async def goto_references(self, file_path: str, content: str, line: int, character: int) -> list[dict]:
        language_id = get_language_id(file_path)
        await self.did_open(file_path, language_id, content)
        ...
        for loc in result if isinstance(result, list) else []:
            references.append({"file": _uri_to_path(loc.get("uri", "")), "line": loc.get("range", {}).get("start", {}).get("line", 0), "character": loc.get("range", {}).get("start", {}).get("character", 0)})
        return references

    async def rename(self, file_path: str, content: str, line: int, character: int, new_name: str) -> dict[str, str] | None:
        language_id = get_language_id(file_path)
        await self.did_open(file_path, language_id, content)
