"""LSP client — JSON-RPC 2.0 over stdio for communicating with language servers."""
...
# Default language server configurations by file extension
...
class LspClient:
    """JSON-RPC 2.0 client over stdio for a single LSP server."""
    ...
    async def stop(self) -> None:
        """Shutdown and stop the LSP server process."""
    ...
    async def get_diagnostics(self, file_path: str, content: str) -> list[dict]:
        """Open file, collect diagnostics, close file. Returns list of diagnostic dicts."""
        language_id = get_language_id(file_path)
        await self.did_open(file_path, language_id, content)

        # Wait briefly for the server to process and publish diagnostics
        await asyncio.sleep(0.5)

        diagnostics: list[dict] = []
        # Drain published diagnostics from the notification queue
        temp: list[dict] = []
    ...
    async def goto_definition(
        ...
        if result is None:
            return []
        # Normalize: can be a single Location or array of Location/LocationLink
    ...
    async def rename(
        ...
        # We apply edits and return {uri: new_text}
...
def _severity_name(severity: int) -> str:
    """Map LSP diagnostic severity to human-readable name."""
...
def _position_to_offset(text: str, line: int, character: int) -> int:
    ...
    for i, current_line in enumerate(lines):
        ...
        offset += len(current_line) + 1  # +1 for \n
    return len(text)
