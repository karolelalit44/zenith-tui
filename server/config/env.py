...
def require_env(key: str) -> str:
    ...
    if val is None or val.strip() == "":
        raise RuntimeError(f"Required environment variable '{key}' is not set. Set it before starting the server.")
    return val.strip()
...
def require_int(key: str) -> int:
    ...
    try:
        ...
        raise RuntimeError(f"Environment variable '{key}' must be an integer, got: {val!r}") from None
