...
def serve(host: str | None, port: int | None):
    ...
    app = create_app()
    uvicorn.run(app, host=host or os.environ.get(HOST_ENV_VAR, DEFAULT_HOST), port=port or int(os.environ.get(PORT_ENV_VAR, str(DEFAULT_PORT))), ws_ping_interval=None, ws_ping_timeout=None)
...
def migrate(db_path: str | None):
    ...
    target = _resolve_db_option(db_path)
    result = DatabaseStartupService(target).run()
    click.echo(f"Migrated: mode={result['mode']} version={result['version']} applied={result['applied']}")
...
def downgrade(db_path: str | None):
    target = _resolve_db_option(db_path)
    click.echo(f"Downgrade is not supported. DB: {target} — " "revert schema changes with a new forward migration instead.")
