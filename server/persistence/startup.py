"""DatabaseStartupService — idempotent schema bring-up on every server start.

Uses the SQL-file migration runner (``server.persistence.migrations.runner``):
migrations are plain ``NNN_title.sql`` files applied in serial order and
tracked in the ``schema_migrations`` table.

State machine (``run``):

1. **Fresh** — DB file missing or 0 bytes. Create it and apply all migrations.
2. **Legacy** — DB has the old ``_migrations`` table but no ``schema_migrations``.
   Its schema already equals the baseline migration ``001``, so we stamp ``001``,
   drop ``_migrations``, then apply the rest (002+).
3. **Alembic** — DB has an ``alembic_version`` table (a DB previously managed by
   the Alembic system). Stamp every migration whose serial <= the recorded
   revision, drop ``alembic_version``, then apply the rest.
4. **Current** — ``schema_migrations`` exists. Apply any pending files.

After migrating, a light parity check verifies every ORM table exists.
"""
...
def get_current_version(db_path: str) -> str | None:
    """Return the highest applied migration serial, or None when not migrated."""
    return runner.get_current_version(db_path)


class DatabaseStartupService:
    """Idempotent migration bring-up for a single SQLite file."""
        ...
        try:
            if self.mode == "fresh":
                ...
                raise MigrationError(
                    f"Database '{db_path}' has tables but no _migrations, "
                    "alembic_version, or schema_migrations — cannot determine "
                    "migration state"
                )
