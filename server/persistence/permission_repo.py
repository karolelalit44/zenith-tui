"""Permission repository — durable storage of permission decisions.

HP-8: grants/denies are written to the `permissions` table so they survive
server restarts and can block tools without an interactive UI round-trip.
"""
...
class PermissionRepository:
    ...
    async def save(self, grant: PermissionGrant) -> None:
        await self.db.execute(
            """INSERT INTO permissions (id, tool_name, decision, session_id, expires_at, created_at)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (
                str(uuid.uuid4()),
                grant.tool_name,
