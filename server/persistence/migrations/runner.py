...
def discover(sql_dir: Path | str | None = None) -> list[MigrationInfo]:
    ...
    for f in sorted(directory.glob("*.sql")):
        ...
        if not m:
            ...
            continue
        migrations.append({"version": m.group(1), "title": m.group(2).replace("_", " "), "filename": f.name, "path": f})
    return migrations
...
def ensure_tracking_table(db_path: str) -> None:
    conn = sqlite3.connect(db_path)
    try:
        conn.execute(f"""CREATE TABLE IF NOT EXISTS {TRACKING_TABLE} (version TEXT PRIMARY KEY, title TEXT NOT NULL DEFAULT '', applied_at TEXT NOT NULL)""")
        conn.commit()
    finally:
        conn.close()
...
def stamp(db_path: str, version: str, title: str = "") -> None:
    ...
    conn = sqlite3.connect(db_path)
    try:
        conn.execute(f"INSERT OR IGNORE INTO {TRACKING_TABLE} (version, title, applied_at) VALUES (?, ?, ?)", (version, title, datetime.now().isoformat()))
        conn.commit()
    finally:
        conn.close()
...
def _apply_file(db_path: str, migration: MigrationInfo) -> None:
    ...
    try:
        ...
        conn.execute(f"INSERT INTO {TRACKING_TABLE} (version, title, applied_at) VALUES (?, ?, ?)", (migration["version"], migration["title"], datetime.now().isoformat()))
        conn.commit()
    finally:
        conn.close()
...
def run_pending(db_path: str) -> list[MigrationInfo]:
    ...
    for m in pending:
        try:
            ...
            db_log("migrate", status="error", version=m["version"], file=m["filename"], error=str(e), db=str(db_path))
            raise MigrationError(f"Migration {m['version']} ({m['filename']}) failed for '{db_path}': {e}", cause=e) from e
