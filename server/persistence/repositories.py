        ...
        async with self.db.session() as s:
            ...
            for p in seed_providers:
                ...
                for m in p["models"]:
                    await s.execute(
                        text(
                            " INSERT INTO provider_models (id, provider_id, name, context_window, description, is_default) VALUES (:id, :provider_id, :name, :context_window, :description, :is_default) ON CONFLICT(provider_id, id) DO NOTHING "
                        ),
                        {
                            "id": m["id"],
                            "provider_id": p["id"],
            ...
            async with self.db.session() as s:
                for prov_id, prov_data in catalog.get("providers", {}).items():
                    for m in prov_data.get("models", []):
                        ...
                        await s.execute(
                            text(
                                "INSERT OR REPLACE INTO pricing (model_id, provider, input_1m, output_1m, cache_read_1m, cache_creation_1m, updated_at) VALUES (:model, :provider, :input, :output, :cache_read, :cache_creation, :updated)"
                            ),
                            {
                                "model": m["id"],
                                "provider": prov_id,
