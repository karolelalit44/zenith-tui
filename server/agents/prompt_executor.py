...
class PromptExecutor:
    ...
    def __init__(self, config: AppSettings, provider: BaseProvider, tool_registry: ToolRegistry, session_repo: SessionRepository, message_repo: MessageRepository, skill_loader: SkillLoader) -> None:
        self._config = config
    ...
    def run(self, session_id: str, content: str, mode: str = "build", handlers: MethodHandlers | None = None, manager=None, model_override: str | None = None, temperature: float | None = None, max_tokens: int | None = None, attachments: list[dict] | None = None) -> None:
        import functools

        self._active_task = asyncio.create_task(self._execute(session_id, content, mode, handlers, manager, model_override, temperature, max_tokens, attachments))
    ...
    def _on_task_done(self, session_id: str, task: asyncio.Task) -> None:
        ...
        exc = task.exception()
        if exc:
            logger.error("BACKGROUND TASK FAILED session=%s error=%s", session_id, exc, exc_info=exc)

    async def _inject_attachments(self, content: str, attachments: list[dict], session_id: str, manager, collected_events: list[Event]) -> str:
        blocks: list[str] = []
        total = 0
        for att in attachments:
            ...
            if total > ATTACHMENT_MAX_TOTAL:
                warning = r.warning(f"Skipped attachment '{path}': total attachment size exceeds {ATTACHMENT_MAX_TOTAL} bytes", session_id)
                collected_events.append(warning)
                if manager:
                    await manager.send_event(session_id, warning)
                continue
        ...
        if not blocks:
            return content
        injected = "\n\n".join(blocks) + "\n\n" + content
        logger.info("Injected %d attachment block(s) into prompt for session %s (content %d -> %d chars)", len(blocks), session_id, len(content), len(injected))
        return injected

    async def _execute(self, session_id: str, content: str, mode: str, handlers: MethodHandlers | None, manager, model_override: str | None = None, temperature: float | None = None, max_tokens: int | None = None, attachments: list[dict] | None = None) -> None:
        logger.info("=" * 60)
        ...
        try:
            ...
            if mode == "build":
                try:
                    ...
                    if session and session.plan_output:
                        ...
                        logger.info("Plan context loaded: %d chars for build session %s (approved=%s)", len(plan_context), session_id, plan_approved)
                except Exception:
            ...
            if (mode == "build" and plan_context and not plan_approved and not self._config.auto_approve_plan and not (content and content.strip())):
                logger.info("Plan not yet approved — emitting PLAN_READY for session %s", session_id)
                plan_ready_event = Event(kind=EventKind.PLAN_READY, data={"plan": plan_context, "session_id": session_id}, session_id=session_id)
                if manager:
            ...
            if attachments:
                content = await self._inject_attachments(content, attachments, session_id, manager, collected_events)
            ...
            if (mode == "build" and plan_context and plan_approved and mode_config and mode_config.sub_agent):
                ...
                async for event in sub_agent.run(session_id=session_id, plan_output=plan_context, user_prompt=content, confirm_callback=_confirm, session_repo=self._session_repo, message_repo=self._message_repo):
                    event_count += 1
            ...
            async for event in agent.process_prompt(content, session_id, history, mode, skills_section=skills_section, confirm_callback=_confirm, plan_context=plan_context, model_override=None, repo_map="" if mode == "plan" else None):
                event_count += 1
                collected_events.append(event)
                if event.kind == EventKind.MESSAGE:
                    ...
                    logger.info(" [TOOL CALL]: tool=%s params=%s", event.data.get("tool", ""), str(event.data.get("params", {})))
                elif event.kind == EventKind.TOOL_RESULT:
                    out = str(event.data.get("output", ""))
                    logger.info(" [TOOL RESULT]: tool=%s success=%s output_len=%d error=%s\n%s", event.data.get("tool", ""), event.data.get("success"), len(out), event.data.get("error", ""), out)

                elif event.kind == EventKind.ERROR:
                    logger.info(" ERROR: message=%s code=%s recoverable=%s", event.data.get("message", ""), event.data.get("code"), event.data.get("recoverable"))
                    if event.data.get("code") == "CONTEXT_EXHAUSTED":
                        try:
                            await token_repo.record_degradation(session_id=session_id, step_index=_step_count, before_tokens=0, after_tokens=0, reason="context_exhausted")
                        except Exception:
                            pass
                elif event.kind == EventKind.SUCCESS:
                    logger.info(" SUCCESS: iterations=%s token_info=%s", event.data.get("iterations"), event.data.get("tokenInfo"))
                        ...
                        try:
                            ...
                            if _step_count > 0:
                                for s in range(1, _step_count + 1):
                                    await token_repo.record(session_id=session_id, provider=provider_name, model=model_name, total_tokens=used // _step_count, context_window=ctx_window, prompt_tokens=prompt_t // _step_count, completion_tokens=completion_t // _step_count, input_tokens=prompt_t // _step_count, output_tokens=completion_t // _step_count, cache_read_tokens=cache_read_t // _step_count, cache_creation_tokens=cache_creation_t // _step_count, step_index=s, estimated=estimated)
                            elif not token_usage_recorded:
                                await token_repo.record(session_id=session_id, provider=provider_name, model=model_name, total_tokens=used, context_window=ctx_window, prompt_tokens=prompt_t, completion_tokens=completion_t, input_tokens=prompt_t, output_tokens=completion_t, cache_read_tokens=cache_read_t, cache_creation_tokens=cache_creation_t, estimated=estimated)
                            token_usage_recorded = True
                            logger.info("Token usage recorded: provider=%s model=%s tokens=%d/%d cache_read=%d cache_creation=%d", provider_name, model_name, used, ctx_window, cache_read_t, cache_creation_t)
                            try:
                    ...
                    if "Context" in msg and ("approaching" in msg or "summarizing" in msg or "exhausted" in msg):
                        try:
                            ...
                            await token_repo.record_degradation(session_id=session_id, step_index=_step_count, before_tokens=deg_ti.get("used", 0), after_tokens=deg_ti.get("remaining", 0), reason=msg[:100])
                        except Exception as e:
            ...
            if mode == "plan" and response_text:
                try:
                    ...
                    if session:
                        ...
                        logger.info("Plan output saved to session %s: %d chars (auto_approve=%s)", session_id, len(response_text), self._config.auto_approve_plan)
                except Exception:
