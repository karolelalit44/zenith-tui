...
from server.api.schemas import (ProviderModelInfo, ValidationError, ValidationResult, ValidationStep, ValidationStepStatus)
...
STEP_LABELS: dict[str, str] = {"config": "Configuration", "base_url": "Base URL", "api_key": "API Key", "connection": "Connection", "auth": "Authentication", "models": "Model Catalog", "smoke_test": "Smoke Test", "save": "Save"}

_URL_RE = re.compile(r"^https?://", re.IGNORECASE)

_AUTH_REJECTION_HINTS = ("authentication", "unauthorized", "invalid api key", "api key invalid", "api_key_invalid", "401", "403")
...
def _step_event(key: str, status: ValidationStepStatus, message: str = "") -> dict:
    return {"type": "step", "key": key, "label": STEP_LABELS.get(key, key), "status": status.value, "message": message}


def _result_event(valid: bool, provider: str, steps: list[ValidationStep], models: list[ProviderModelInfo], error: ValidationError | None) -> dict:
    payload = ValidationResult(valid=valid, provider=provider, steps=steps, models=models, error=error)
...
def _map_models(raw: Any) -> list[ProviderModelInfo]:
    ...
    for item in data:
        ...
        if not mid or mid in seen:
            continue
        seen.add(mid)
        out.append(ProviderModelInfo(id=mid, name=str(item.get("name") or mid), context_window=int(item.get("context_window") or item.get("context_length") or 128000), description=str(item.get("description") or ""), is_default=bool(item.get("is_default", False)), status=str(item.get("status") or "active"), tags=[str(t) for t in item.get("tags", [])] if isinstance(item.get("tags"), list) else []))
    return out
...
def _resolve_config(provider_id: str, api_key: str, base_url: str, model: str, db_path: str) -> tuple[dict, dict]:
    ...
    config = {"api_key": resolved_key, "base_url": resolved_base, "model": resolved_model, "max_tokens": int(stored.get("max_tokens") or 4096), "temperature": float(stored.get("temperature") or 0.7), "adapter": entry.get("adapter", "openai_compat"), "litellm_prefix": entry.get("litellm_prefix", ""), "requires_api_key": bool(entry.get("requires_api_key", True)), "api_key_prefix": entry.get("api_key_prefix")}
    return entry, config
...
async def validate_provider(provider_id: str, api_key: str = "", base_url: str = "", model: str = "", db_path: str | None = None, workspace_root: str = ".") -> AsyncIterator[dict]:
    db_path = db_path or provider_config_repo.resolve_db_path()
    steps: dict[str, ValidationStep] = {key: ValidationStep(key=key, label=label) for key, label in STEP_LABELS.items()}
    ...
    if cfg["requires_api_key"] and api_key.strip() and probe_model:
        try:
            ...
            probe_provider = LLMProvider(name=provider_id, api_key=api_key, base_url=base_url, model=probe_model, max_tokens=1, temperature=0.0, enable_thinking=False)
            await asyncio.wait_for(probe_provider.complete([{"role": "user", "content": "Say OK"}]), timeout=timeout)
        except ImportError:
    ...
    if not auth_ok:
        ...
        yield _result_event(False, provider_id, _all_steps(), [], ValidationError(code="AUTH_FAILED", message=auth_msg))
        return
    ...
    if smoke_model:
        try:
            ...
            litellm.drop_params = True
            temp_provider = LLMProvider(name=provider_id, api_key=api_key, base_url=base_url, model=smoke_model, max_tokens=cfg["max_tokens"], temperature=cfg["temperature"])
            await asyncio.wait_for(temp_provider.complete([{"role": "user", "content": "Say OK"}]), timeout=timeout)
        except ImportError:
    ...
    if smoke_error:
        ...
        yield _result_event(False, provider_id, _all_steps(), models, ValidationError(code="SMOKE_TEST_FAILED", message=smoke_error))
        return
    ...
    try:
        provider_config_repo.save_provider_config(provider=provider_id, api_key=api_key, model=smoke_model, base_url=base_url, max_tokens=cfg["max_tokens"], temperature=cfg["temperature"], db_path=db_path, set_active=False)
        if entry.get("custom_flow") and models:
            provider_config_repo.upsert_provider_models(provider=provider_id, models=[m.model_dump() for m in models], db_path=db_path)
    except Exception as e:
...
async def validate_provider_collect(provider_id: str, api_key: str = "", base_url: str = "", model: str = "", db_path: str | None = None, workspace_root: str = ".") -> ValidationResult:
