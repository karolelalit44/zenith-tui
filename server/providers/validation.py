"""Provider validation pipeline — guided live validation with NDJSON streaming.

Validates a provider configuration through 8 sequential steps:

1. ``config``      — resolve the merged config (request overrides > stored > catalog default)
2. ``base_url``    — base URL is present and a valid http(s) endpoint
3. ``api_key``     — key present (unless optional) and matches the provider's expected prefix
4. ``connection``  — endpoint reachable (any HTTP response counts as reachable)
5. ``auth``        — request authenticates (not 401/403)
6. ``models``      — discover the live model catalog from ``GET {base_url}/models``
7. ``smoke_test``  — real completion round-trip (``Say OK``)
8. ``save``        — persist config + models, set active, mark validated

The pipeline is an async generator yielding NDJSON-friendly events so the TUI can
render a live progress view. ``validate_provider_collect`` is a sync convenience
wrapper that consumes the stream and returns a single ``ValidationResult``.
"""
...
def _map_models(raw: Any) -> list[ProviderModelInfo]:
    """Normalize a live ``GET /models`` payload into ProviderModelInfo list."""
...
def _base_url_endpoint(base_url: str) -> str:
    """Normalize base_url so a trailing '/' never breaks the /models suffix."""
