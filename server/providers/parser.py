...
TOOL_PATTERNS = [re.compile(r"```(?:tool|json)?\s*\n?(\{[\s\S]*?\"tool\"\s*:\s*\"[^\"]+\"[\s\S]*?\})\s*\n?```", re.IGNORECASE), re.compile(r"(\{[\s\S]*?\"tool\"\s*:\s*\"[^\"]+\"[\s\S]*?\"params\"\s*:\s*\{[\s\S]*?\}\s*\})"), re.compile(r"```(?:tool|json)?\s*\n?(\[[\s\S]*?\])\s*\n?```", re.IGNORECASE), re.compile(r"(\[[\s\S]*?\{[\s\S]*?\"tool\"\s*:\s*\"[^\"]+\"[\s\S]*?\}[\s\S]*?\])")]
...
XML_ARG_PAIR_PATTERN = re.compile(r"<arg_key>(.*?)</arg_key>\s*<arg_value>(.*?)</arg_value>", re.DOTALL | re.IGNORECASE)
...
def clean_tool_text(text: str) -> str:
    ...
    cleaned = re.sub(r"\{[\s\S]*?\"tool\"\s*:\s*\"[^\"]+\"[\s\S]*?\"params\"\s*:\s*\{[\s\S]*?\}\s*\}", "", cleaned)
    cleaned = re.sub(r"```(?:tool|json)?\s*\n?\[[\s\S]*?\"tool\"\s*:\s*\"[^\"]+\"[\s\S]*?\]\s*\n?```", "", cleaned, flags=re.IGNORECASE)
    cleaned = re.sub(r"\[[\s\S]*?\{[\s\S]*?\"tool\"\s*:\s*\"[^\"]+\"[\s\S]*?\}[\s\S]*?\]", "", cleaned)
    cleaned = re.sub(r"\[(\w+)((?:\s+\w+=(?:\"[^\"]*\"|\S+))+\s*)\]", "", cleaned)
