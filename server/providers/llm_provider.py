...
from .base import (BaseProvider, ModelInfo, ProviderChunk, ProviderResponse, TokenUsage, ToolCall, ToolCallDelta)
...
def _classify_provider_error(exc: Exception, provider_name: str) -> ProviderError:
    ...
    is_quota_exhausted = any(q in msg for q in ("free-models-per-day", "insufficient_quota", "credit_balance", "payment_required", "quota_exceeded", "add 10 credits"))

    try:
        import litellm

        if isinstance(exc, litellm.ContextWindowExceededError):
            return ProviderError(f"Context window exceeded for provider '{provider_name}': {clean}", provider=provider_name, code="CONTEXT_EXCEEDED", recoverable=True)
        if isinstance(exc, litellm.RateLimitError):
            return RateLimitError(f"Rate limited by provider '{provider_name}': {clean}", provider=provider_name, retry_after=retry_after, recoverable=not is_quota_exhausted)
        if isinstance(exc, litellm.AuthenticationError):
            return AuthenticationError(f"Authentication failed for provider '{provider_name}': {clean}\nTip: Check your API key is set correctly in settings.", provider=provider_name)
...
def _map_finish_reason(raw: str | None) -> FinishReason:
    ...
    mapping = {"stop": FinishReason.STOP, "length": FinishReason.LENGTH, "tool_calls": FinishReason.TOOL_CALLS, "content_filter": FinishReason.CONTENT_FILTER, "function_call": FinishReason.TOOL_CALLS}
    ...
    try:
        ...
        for m in provider_entry.get("models", []):
            if m["id"] == model_id:
                ...
                return {"context_window": ctx, "max_output_tokens": m.get("max_output_tokens", min(ctx // 4, 32768)), "default_temperature": m.get("default_temperature", 0.0 if caps.get("reasoning") else 0.7), "enable_thinking": caps.get("thinking", False), "supports_tools": caps.get("function_calling", True), "use_system_prompt": m.get("use_system_prompt", True), "streaming": m.get("streaming", True), "extra_params": m.get("extra_params", None), "edit_format": m.get("edit_format", "tool" if caps.get("function_calling") else "diff")}
    except Exception:
        pass
    return {"context_window": 128000, "max_output_tokens": 4096, "default_temperature": 0.7, "enable_thinking": False, "supports_tools": True, "use_system_prompt": True, "streaming": True, "extra_params": None, "edit_format": "tool"}


class LLMProvider(BaseProvider):

    def __init__(self, name: str, model: str | None = None, max_tokens: int | None = None, temperature: float | None = None, api_key: str | None = None, base_url: str | None = None, enable_thinking: bool | None = None, reasoning_budget: int | None = None, extra_params: dict | None = None, use_system_prompt: bool | None = None, streaming: bool | None = None, weak_model: str | None = None):
        catalog = _get_catalog()
        provider_entry = catalog["providers"].get(name, {})
        resolved_model = model or provider_entry.get("default_model")
        if not resolved_model:
            raise ValueError(f"No model specified and no default_model in catalog for provider '{name}'. " f"Provide a model explicitly or add one to the provider catalog.")
        ...
        self._supports_thinking_headers = bool(provider_entry.get("supports_thinking_headers", False))
        ...
        logger.info("LLMProvider init: name=%s model=%s litellm_model=%s max_tokens=%d temperature=%.2f " "use_system_prompt=%s streaming=%s edit_format=%s extra_params=%s", name, self.model, self._litellm_model, self.max_tokens, self.temperature, self.use_system_prompt, self.streaming_enabled, self.edit_format, self.extra_params is not None)

    def _build_completion_kwargs(self, messages: list[dict], tools: list[dict] | None = None, stream: bool = False, tool_choice: str | None = None, response_format: dict | None = None, model_override: str | None = None) -> dict:
        ...
        kwargs: dict = {"model": litellm_model, "messages": messages, "max_tokens": self.max_tokens, "temperature": self.temperature, "stream": stream and self.streaming_enabled, "num_retries": 1}
    ...
    async def _complete_impl(self, messages: list[dict], tools: list[dict] | None = None, model: str | None = None) -> str:
        ...
        try:
            ...
            logger.info("API RESPONSE (complete) model=%s elapsed=%.0fms finish=%s content_len=%d usage=%s", self._litellm_model, elapsed, raw_finish, len(content), f"prompt={getattr(usage, 'prompt_tokens', '?')} completion={getattr(usage, 'completion_tokens', '?')}" if usage else "none")
            logger.info("API RESPONSE CONTENT: %r", content)

            usage = getattr(response, "usage", None)
            if usage:
                self._last_usage = {"prompt_tokens": getattr(usage, "prompt_tokens", 0) or 0, "completion_tokens": getattr(usage, "completion_tokens", 0) or 0, "total_tokens": getattr(usage, "total_tokens", 0) or 0}
            ...
            if tool_calls:
                self._last_native_tool_calls = [{"id": tc.id, "type": "function", "function": {"name": tc.function.name, "arguments": tc.function.arguments}} for tc in tool_calls]
    ...
    async def _stream_impl(self, messages: list[dict], tools: list[dict] | None = None, tool_choice: str | None = None, response_format: dict | None = None) -> AsyncIterator[tuple[str, str | None]]:
        import litellm
        ...
        async for chunk in stream:
            ...
            if hasattr(chunk, "usage") and chunk.usage:
                u = chunk.usage
                stream_usage = {"prompt_tokens": getattr(u, "prompt_tokens", 0) or 0, "completion_tokens": getattr(u, "completion_tokens", 0) or 0, "total_tokens": getattr(u, "total_tokens", 0) or 0}
            ...
            if delta.tool_calls:
                for tc_delta in delta.tool_calls:
                    ...
                    if idx not in accumulated_tool_calls:
                        accumulated_tool_calls[idx] = {"id": tc_delta.id or "", "type": "function", "function": {"name": "", "arguments": ""}}
        ...
        logger.info("API STREAM DONE model=%s elapsed=%.0fms chunks=%d content=%d reasoning=%d tools=%d finish=%s usage=%s", self._litellm_model, elapsed, chunk_count, content_chars, reasoning_chars, len(accumulated_tool_calls), finish, stream_usage)
    ...
    async def complete_typed(self, messages: list[dict], tools: list[dict] | None = None, temperature: float | None = None, max_tokens: int | None = None) -> ProviderResponse:
        self._reset_cumulative_usage()
        ...
        try:
                ...
                for tc in raw_tool_calls:
                    ...
                    if isinstance(args, str):
                        try:
                            ...
                            args = {}
                    tool_calls.append(ToolCall(id=tc.id, name=tc.function.name, arguments=args if isinstance(args, dict) else {}, raw_arguments=tc.function.arguments if isinstance(tc.function.arguments, str) else ""))
            ...
            if hasattr(response, "usage") and response.usage:
                ...
                usage = TokenUsage(prompt_tokens=getattr(response.usage, "prompt_tokens", 0) or 0, completion_tokens=getattr(response.usage, "completion_tokens", 0) or 0, total_tokens=getattr(response.usage, "total_tokens", 0) or 0, cached_tokens=cached)
                self._last_usage = {"prompt_tokens": usage.prompt_tokens, "completion_tokens": usage.completion_tokens, "total_tokens": usage.total_tokens, "cached_tokens": cached, "cache_creation_tokens": cache_creation}
                or_cost = _extract_openrouter_cost(response)
                if or_cost is not None:
                    self._last_usage["_override_cost"] = or_cost
                self._accumulate_usage(self._last_usage)
            ...
            return ProviderResponse(content=content, tool_calls=tool_calls, usage=usage, finish_reason=finish_reason, model=self.model, provider=self.name)
        except ProviderError:
            raise
        except Exception as e:
            raise _classify_provider_error(e, self.name) from e
    ...
    async def stream_typed(self, messages: list[dict], tools: list[dict] | None = None) -> AsyncIterator[ProviderChunk]:
    ...
    async def _stream_typed_impl(self, messages: list[dict], tools: list[dict] | None = None) -> AsyncIterator[ProviderChunk]:
        import litellm
        ...
        async for chunk in stream:
            ...
            reasoning = getattr(delta, "reasoning_content", None)
            if reasoning:
                yield ProviderChunk(delta="", tool_call_delta=ToolCallDelta(index=-1, id=None, name=None, arguments_delta=reasoning))
            ...
            if delta.tool_calls:
                for tc_delta in delta.tool_calls:
                    ...
                    if tc_delta.function:
                        ...
                        if tc_delta.function.arguments:
                            tc["function"]["arguments"] += tc_delta.function.arguments
                            yield ProviderChunk(tool_call_delta=ToolCallDelta(index=idx, name=tc_delta.function.name, arguments_delta=tc_delta.function.arguments))
    ...
    def list_models_typed(self) -> list[ModelInfo]:
        try:
            ...
            for m in models:
                ...
                result.append(ModelInfo(id=m["id"], name=m.get("name", m["id"]), provider=self.name, max_tokens=max_out, context_window=ctx, supports_tools=caps.get("function_calling", True), supports_thinking=caps.get("thinking", False), supports_vision="image" in m.get("input_modalities", []) or "image" in m.get("output_modalities", []), streaming=m.get("streaming", True), use_system_prompt=m.get("use_system_prompt", True), edit_format=m.get("edit_format", "tool" if caps.get("function_calling") else "diff")))
            return result
        except Exception as e:
            logger.warning("Could not list models from catalog for provider '%s': %s", self.name, e)
