...
class TestRegistryDefaults:
    def test_default_registry_has_codebase_scout(self):
        registry = SpecialistRegistry.default()
        scout = registry.get("codebase-scout")
        assert scout is not None
        assert scout.role == "Codebase Investigator"
    ...
    def test_route_demo_prompt_via_persistence_analysis(self):
        ...
        assert routed.id == "codebase-scout"

    def test_route_score_at_or_above_threshold(self):
        assert score_prompt(DEMO_PROMPT, CodebaseScout) >= MIN_CAPABILITY_SCORE
...
class TestAvoidFor:
    ...
    def test_avoid_match_detects_fully_matched_phrase(self):
        assert avoid_match("add a test for file_write", CodebaseScout) is True
...
class TestScoring:
    def test_scoring_is_deterministic(self):
        s1 = score_prompt(DEMO_PROMPT, CodebaseScout)
        s2 = score_prompt(DEMO_PROMPT, CodebaseScout)
        assert s1 == s2
        assert s1 > 0

    def test_best_for_and_avoid_for_accessors(self):
        registry = SpecialistRegistry.default()
        assert "investigate" in registry.best_for("codebase-scout")
        assert "migrate now" in registry.avoid_for("codebase-scout")
