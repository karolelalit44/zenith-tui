...
class HookMiddleware(ToolMiddleware):
    ...
    async def before_execute(self, name: str, params: dict[str, Any], ctx: ToolContext) -> bool | ToolResult:
        if not self._runner.pre_tool_use:
            return True
        results = await self._runner.run_pre_tool_use(name, params, workspace_root=ctx.workspace_root or ".", session_id=ctx.session_id, mode=ctx.mode)
        for r in results:
            if r["exit_code"] != 0:
    ...
    async def after_execute(self, name: str, params: dict[str, Any], result: ToolResult, ctx: ToolContext) -> ToolResult:
        if not self._runner.post_tool_use:
            return result
        try:
            results = await self._runner.run_post_tool_use(name, params, result, workspace_root=ctx.workspace_root or ".", session_id=ctx.session_id, mode=ctx.mode)
