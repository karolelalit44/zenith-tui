...
class BashTool(BaseTool):
    ...
    def get_schema(self) -> dict:
        return {"type": "object", "properties": {"command": {"type": "string", "description": "Command to execute"}, "timeout": {"type": "integer", "description": "Timeout seconds", "default": 30}, "run_in_background": {"type": "boolean", "description": "Run in background", "default": False}, "auto_background_after": {"type": "integer", "description": "Background delay seconds", "default": 60}}, "required": ["command"]}
    ...
    async def _start_background(self, command: str, workspace_root: str, description: str) -> ToolResult:
        ...
        if job.done:
            ...
            return ToolResult(success=job.exit_code == 0, output=output, error="" if job.exit_code == 0 else output, metadata={"exit_code": job.exit_code, "background": False, "job_id": job.id})

        return ToolResult(success=True, output=(f"Background job started with ID: {job.id}\n" f"Command: {command}\n\n" f"Use job_output tool to view output or job_kill to terminate."), metadata={"background": True, "job_id": job.id})

    async def _execute_sync(self, command: str, workspace_root: str, timeout: int, auto_background_after: int) -> ToolResult:
        process = None
        try:
            shell = _resolve_shell()

            if _IS_WINDOWS and shell:
                process = await asyncio.create_subprocess_shell(f'"{shell}" -NoProfile -Command {command}', stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, cwd=workspace_root)
            else:
                process = await asyncio.create_subprocess_shell(command, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE, cwd=workspace_root, shell=True)
            ...
            try:
                ...
                if process.returncode is None:
                    ...
                    return ToolResult(success=True, output=(f"Command is taking longer than expected and has been moved to background.\n" f"Background job ID: {job.id}\n" f"Command: {command}\n\n" f"Use job_output tool to view output or job_kill to terminate."), metadata={"background": True, "job_id": job.id})
                else:
            ...
            if exit_code == 0:
                ...
                return ToolResult(success=False, output=output, error=error, metadata={"exit_code": exit_code})
