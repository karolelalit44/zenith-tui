...
class FileEditTool(BaseTool):
    ...
    def get_schema(self) -> dict:
        return {"type": "object", "properties": {"path": {"type": "string", "description": "File path"}, "old_content": {"type": "string", "description": "Text to replace"}, "new_content": {"type": "string", "description": "Replacement text"}}, "required": ["path", "old_content", "new_content"]}
    ...
    async def execute(self, params: dict[str, Any], workspace_root: str) -> ToolResult:
        ...
        try:
            ...
            if old in content:
                ...
                if count > 1:
                    return ToolResult(success=False, error=f"Ambiguous: found {count} matches. Provide more surrounding context.")
                new_content = content.replace(old, new, 1)
                resolved.write_text(new_content, encoding="utf-8")
                return ToolResult(success=True, output=f"Edited {rel_path}", metadata={"path": str(resolved), "changes": 1, "match": "exact"})
            ...
            if fuzzy_result is not None:
                ...
                resolved.write_text(new_content, encoding="utf-8")
                return ToolResult(success=True, output=f"Edited {rel_path} (fuzzy match, {ratio:.0%} similarity)", metadata={"path": str(resolved), "changes": 1, "match": "fuzzy", "similarity": round(ratio, 3)})

            preview = old[:80] + ("..." if len(old) > 80 else "")
            return ToolResult(success=False, error=f"Content not found in file (tried exact + fuzzy matching): {preview}")
