"""Todo tool — allows the agent to track tasks and maintain a structured task list.

The agent can create, update, list, and complete tasks.  This helps with
multi-step workflows where the agent needs to track progress across
multiple tool calls.
"""
...
@dataclass
class Task:
    """A tracked task with status and priority."""
...
class TaskTracker:
    """Manages a list of tracked tasks for the current session."""
    ...
    def format_summary(self) -> str:
        """Format a human-readable summary of all tasks."""
        if not self._tasks:
...
# Singleton task tracker for the current session
_task_tracker: TaskTracker | None = None


def get_task_tracker() -> TaskTracker:
    """Get the global task tracker singleton."""
