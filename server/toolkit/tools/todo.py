...
class TodoTool(BaseTool):
    ...
    def get_schema(self) -> dict:
        return {"type": "object", "properties": {"action": {"type": "string", "description": "Action: add, update, list, summary", "enum": ["add", "update", "list", "summary"]}, "task_id": {"type": "string", "description": "Task ID"}, "description": {"type": "string", "description": "Task description"}, "status": {"type": "string", "description": "Status: pending, in_progress, completed, cancelled", "enum": ["pending", "in_progress", "completed", "cancelled"]}, "priority": {"type": "string", "description": "Priority: low, medium, high", "enum": ["low", "medium", "high"], "default": "medium"}}, "required": ["action"]}
    ...
    async def execute(self, params: dict[str, Any], workspace_root: str) -> ToolResult:
        ...
        if action == "add":
            ...
            return ToolResult(success=True, output=f"Task {task.id} added: {task.description} (priority: {task.priority})", metadata={"task_id": task.id, "action": "add"})
            ...
            return ToolResult(success=True, output=f"Task {task.id} updated: {task.description} (status: {task.status})", metadata={"task_id": task.id, "action": "update"})
