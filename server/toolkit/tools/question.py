...
class QuestionTool(BaseTool):
    ...
    def get_schema(self) -> dict:
        return {"type": "object", "properties": {"question": {"type": "string", "description": "Question text"}, "options": {"type": "array", "description": "Multiple-choice options", "items": {"type": "string"}}}, "required": ["question"]}
    ...
    async def execute(self, params: dict[str, Any], workspace_root: str) -> ToolResult:
        ...
        if _question_callback is None:
            return ToolResult(success=True, output=f"[Question asked but no interactive handler available]\n{question}", metadata={"question": question, "options": options, "answered": False})

        try:
            answer = await _question_callback(question, options)
            return ToolResult(success=True, output=f"User answered: {answer}", metadata={"question": question, "options": options, "answer": answer, "answered": True})
