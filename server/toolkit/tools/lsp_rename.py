...
class LspRenameTool(BaseTool):
    ...
    def get_schema(self) -> dict:
        return {"type": "object", "properties": {"filepath": {"type": "string", "description": "File path"}, "line": {"type": "integer", "description": "Line (0-indexed)"}, "character": {"type": "integer", "description": "Column (0-indexed)"}, "new_name": {"type": "string", "description": "New symbol name"}}, "required": ["filepath", "line", "character", "new_name"]}
    ...
    async def execute(self, params: dict[str, Any], workspace_root: str) -> ToolResult:
        ...
        if not file_edits:
            return ToolResult(success=True, output=f"No rename possible at {path.name}:{line + 1}:{character + 1}. Symbol may not exist or server does not support rename.", metadata={"changed_files": 0})
        ...
        for edit_path, new_content in file_edits.items():
            try:
                ...
                return ToolResult(success=False, error=f"Failed to write {edit_path}: {e}. Changes rolled back.", metadata={"changed_files": 0, "partial": True})

        lines = [f"Renamed '{path.stem}' symbol to '{new_name}' in {len(changed_files)} file(s):"]
        for f in changed_files:
            lines.append(f"  {f}")

        return ToolResult(success=True, output="\n".join(lines), metadata={"changed_files": changed_files, "new_name": new_name})
