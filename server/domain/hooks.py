...
Hook commands are configured via ``AppSettings.hooks`` (or the ``ZENITH_HOOKS``
JSON env var). Each command runs as a shell subprocess in the workspace root
with a JSON payload on stdin and template fields substituted into the command
string.

Exit-code semantics:
  PreToolUse:   non-zero exit blocks the tool (middleware returns a ToolResult).
  PostToolUse:  runs after execution; non-zero exit is logged only.
  SessionStart: runs after a session is created; non-zero exit is logged only.
"""
...
class HookRunner:
    """Runs configured shell hooks with exit-code semantics."""
