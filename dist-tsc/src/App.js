...
export const App = () => {
    ...
    const handleRetry = useCallback(() => {
        if (activeTurn && !isRunning)
            startScenario(activeTurn.prompt, activeTurn.mode, activeProvider.id);
    ...
    if (startupState.phase === 'loading') {
        return (React.createElement(Box, { flexDirection: "column", paddingX: 1, paddingTop: 1, width: "100%", justifyContent: "center", alignItems: "center", minHeight: 5 },
    ...
    if (startupState.phase === 'setup' || startupState.phase === 'error') {
        return (React.createElement(Box, { flexDirection: "column", paddingX: 1, paddingTop: 1, width: "100%" },
            React.createElement(SetupWizard, { startupState: startupState, onComplete: handleSetupComplete })));
    }
    return (React.createElement(AppProvider, { turns: turns, activeTurn: activeTurn, totalTokens: totalTokens, events: events, isRunning: isRunning, overlay: overlay, isOverlayOpen: isOverlayOpen, selectedMode: selectedMode, thinkingCollapsed: thinkingCollapsed, activeConfirmation: activeConfirmation },
        React.createElement(Box, { flexDirection: "column", paddingX: 1, paddingTop: 1, width: "100%" },
            turns.length === 0 && !isRunning && React.createElement(WelcomeScreen, { workspace: workspace }),
            React.createElement(Static, { key: staticKey, items: completedTurns }, (turn, idx) => (React.createElement(Box, { key: turn.id, flexDirection: "column", width: "100%" },
                idx > 0 && (React.createElement(Box, { marginTop: 1, marginBottom: 0, paddingX: 1, width: "100%" },
                ...
                React.createElement(Box, { marginTop: 1, flexDirection: "column", width: "100%" },
                    React.createElement(UserMessageBlock, { prompt: turn.prompt }),
                    turn.events.length > 0 && (React.createElement(ScenarioRenderer, { events: turn.events, isRunning: false, isHistorical: true, thinkingCollapsed: thinkingCollapsed })))))),
