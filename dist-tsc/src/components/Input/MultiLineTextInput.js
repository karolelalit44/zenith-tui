...
const MIN_LINES = 1;
const MAX_LINES = 15;
export const MultiLineTextInput = React.memo(({ value, onChange, onSubmit, placeholder = '', focus = true, historyUp, historyDown }) => {
    ...
    if (value.length === 0) {
        content = (React.createElement(Text, null,
            React.createElement(Text, { inverse: true, color: theme.colors.text.bright }, placeholder ? placeholder[0] : ' '),
            React.createElement(Text, { color: theme.colors.text.dim }, placeholder ? placeholder.slice(1) : '')));
    }
    else {
        ...
        content = lines.map((line, lineIdx) => {
            ...
            if (isCursorLine) {
                ...
                return (React.createElement(Text, { key: lineIdx, wrap: "wrap" },
                    before,
                    React.createElement(Text, { inverse: true }, charAtCursor || ' '),
                    after));
            }
            return (React.createElement(Text, { key: lineIdx, wrap: "wrap" }, line || ' '));
        });
    }
    return (React.createElement(Box, { width: "100%", height: lineCount }, content));
});
