...
export function useConversation() {
    ...
    const totalTokens = useMemo(() => {
        return turns.reduce((sum, t) => {
            ...
            const successEvent = t.events.find((e) => e.kind === 'success' && 'tokenInfo' in e && Boolean(e.tokenInfo));
    ...
    const addTurn = useCallback((prompt, mode) => {
        const now = new Date();
    ...
    const completeActiveTurn = useCallback((events) => {
        setTurns((prev) => {
    ...
    const abortActiveTurn = useCallback(() => {
        setTurns((prev) => {
            ...
            if (last && !last.isComplete) {
                ...
                return prev.map((t, i) => i === prev.length - 1 ? { ...t, events: [...t.events, abortEvent], isComplete: true } : t);
            }
            return prev;
        });
    }, []);
