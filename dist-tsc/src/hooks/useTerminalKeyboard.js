...
export function useTerminalKeyboard({ turns, isRunning, events, overlay, openOverlay, closeOverlay, closeAllOverlays, abort, abortActiveTurn, markTurnSaved, clearTurns, onToggleThinking, activeConfirmation, respondConfirmation, }) {
    ...
    useInput((_char, key) => {
        ...
        if (key.escape) {
            if (opts.overlay !== 'none') {
                if (opts.closeAllOverlays)
                    opts.closeAllOverlays();
                else if (opts.closeOverlay)
                    opts.closeOverlay();
                return;
            }
            if (opts.isRunning) {
        ...
        if ((key.ctrl || key.meta) && opts.overlay === 'none') {
            ...
            if (_char === 'p' || _char === 'P') {
                if (opts.openOverlay)
                    opts.openOverlay('help');
                return;
            }
            if (_char === 'm' || _char === 'M') {
                if (opts.openOverlay)
                    opts.openOverlay('mode');
                return;
            }
            if (_char === 't' || _char === 'T' || _char === '\x14') {
                if (opts.onToggleThinking)
