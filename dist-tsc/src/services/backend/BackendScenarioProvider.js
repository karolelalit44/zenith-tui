...
export class BackendScenarioProvider {
    ...
    execute(_scenario, onEvent, onComplete) {
        ...
        let lastPartialMessageIndex = null;
        let accumulatedText = '';
        let completed = false;
        ...
        const unsubscribe = wsClient.onEvent((rpcEvent) => {
            ...
            if (kind === 'thinking' && lastEventKind === 'thinking' && eventIndex > 0) {
                ...
                const newThoughts = mapped.thoughts;
                for (const t of newThoughts) {
                    const text = typeof t === 'string' ? t : t.text;
                    if (text)
                        mergedThinkingThoughts.push(text);
                }
...
function mapRawEvent(kind, data, rpcId) {
    const d = data || {};
    const id = rpcId || uid();
    switch (kind) {
        case 'thinking':
        ...
        case 'success':
            return {
                ...
                tokenInfo: d.tokenInfo && typeof d.tokenInfo === 'object'
                    ? {
                        used: Number(d.tokenInfo.used) || 0,
                        remaining: Number(d.tokenInfo.remaining) || 0,
                        total: Number(d.tokenInfo.total) || 0,
                        percent: Number(d.tokenInfo.percent) || 0,
                    }
                    : undefined,
            };
        case 'progress':
